        ...
        <div class="card shadow-sm border-0">
            <div class="table-responsive">
                <table class="table card-table table-vcenter text-nowrap table-striped">
                    ...
                    <tbody>
                        <?php 
                        // El secreto está en este ORDER BY para que no se voltee la lógica
                        $res_evals = $conexion->query("SELECT * FROM evaluaciones WHERE id_socio = '$id_socio' ORDER BY fecha_evaluacion DESC, id_evaluacion DESC");
                        $evaluaciones = $res_evals->fetch_all(MYSQLI_ASSOC);

                        if(count($evaluaciones) > 0): 
                            foreach($evaluaciones as $index => $ev): 
                                // El registro anterior en el tiempo es el siguiente en el array
                                $prev = isset($evaluaciones[$index + 1]) ? $evaluaciones[$index + 1] : null;
                                $diff_peso = $prev ? $ev['peso'] - $prev['peso'] : 0;
                        ?>
                        <tr>
                            <td><strong><?php echo date('d/m/Y', strtotime($ev['fecha_evaluacion'])); ?></strong></td>
                            <td class="font-weight-bold"><?php echo number_format($ev['peso'], 2); ?> kg</td>
                            <td>
                            ...
                            <td>
                                <?php if($prev): ?>
                                    <?php if($diff_peso < 0): ?>
                                        <span class="text-green font-weight-bold"><i class="ti ti-trending-down"></i> Bajó <?php echo abs($diff_peso); ?>kg</span>
                                    <?php elseif($diff_peso > 0): ?>
                                        <span class="text-red font-weight-bold"><i class="ti ti-trending-up"></i> Subió <?php echo $diff_peso; ?>kg</span>
                                    <?php else: ?>
                                        <span class="text-muted">Sin cambios</span>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <span class="badge bg-gray-lt">Registro Inicial</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
